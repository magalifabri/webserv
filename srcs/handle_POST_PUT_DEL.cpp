...
std::string find_untaken_name(const std::string &upload_path)
{
	...
	// std::cout << RED BOLD "filename: " << filename << RESET << std::endl; // TESTING
...
std::string handle_POST_request(webserv& webserv, location location, request request)
{
	...
	if (request.body_data.empty() == true)
		return (create_2xx_response_page("200 OK"));
	...
	return (create_2xx_response_page("201 Created", location_header));
}
...
std::string handle_PUT_request(const location &location, request &request)
{
	...
	std::string filename = request.headers_map["full-target"]; // contains leading slash
	// std::string path = location.root + "/";
	std::string path = location.root; // floor
...
std::string	handle_DELETE_request(const location &location, request &request)
{
	...
	std::cout << BOLD RED "DELETE request for: " << target << RESET << std::endl;
